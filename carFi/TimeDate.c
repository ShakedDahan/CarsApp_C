...
char* time_string(Time time)
{
    ...
    {
        char* timeInString;
        timeInString = (char*)malloc(6);
        if(timeInString == NULL) return NULL;
        sprintf(timeInString, "%02d:%02d",time.hours,time.minutes);
        return timeInString;
    }
    return NULL;
}
...
char* date_string(Date date)
{
    ...
    {
        char* dateInString;
        dateInString = (char*)malloc(11);
        if(dateInString == NULL) return NULL;
        sprintf(dateInString, "%02d/%02d/%04d",date.day,date.month,date.year);
        return dateInString;
    }
    return NULL;
}
    ...
    if(date != NULL)
    {
        if(strlen(date) == 10)
        {
            if(date[2] == '/' && date[5] == '/')
            {
                ...
                month = (char*)malloc(3);
                year = (char*)malloc(5);
                if(!day || !month || !year) return def_date();
                for(i=0;i<2;i++)
                {
                    day[i] = date[i];
                }
                ...
                year[5] = '\0';
                newDate = gen_date(atoi(year),atoi(month),atoi(day));
                free(day);
                free(month);
                free(year);
                day = month = year = NULL;
    ...
    if(time != NULL)
    {
        if(strlen(time) == 5)
        {
            if(time[2] == ':')
            {
                ...
                if(!hours || !minutes) return def_time();
                for(i=0;i<2;i++)
                {
                    hours[i] = time[i];
                }
                ...
                minutes[2] = '\0';
                newTime = gen_time(atoi(hours),atoi(minutes));
                free(hours);
                free(minutes);
                hours = NULL;
                minutes = NULL;
